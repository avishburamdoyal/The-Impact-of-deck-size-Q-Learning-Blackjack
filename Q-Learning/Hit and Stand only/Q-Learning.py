...
episodes_to_learn = range(200, 3200, 200)
...
for episode_to_learn in episodes_to_learn:
    total_payoff = 0 # to store total payout over 'num_rounds'
    average_payoff = [] # to store total payout over 'num_rounds' after 'num_sample' simulations
    q_agent = RL_Agent(environment=environment, epsilon=1.0, alpha=0.5, gamma=0.95, num_training_episodes=episode_to_learn)
    ...
    plt.plot(average_payoff, linestyle = 'solid')
    print ("Average payout after {} rounds after training for {} episodes is {}".format(num_rounds, episode_to_learn, total_payoff/(num_samples)))
    

# Plot payout per 1000 episodes for each value of 'num_episodes_to_train' in the same graph
plt.xlabel('Number of samples')
plt.ylabel('Average payoff')
plt.legend(["{}".format(i) for i in episodes_to_learn], loc='upper left')
