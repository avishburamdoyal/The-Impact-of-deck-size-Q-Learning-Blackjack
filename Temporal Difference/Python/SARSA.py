...
def SARSA(environment, episodes, epsilon, alpha, gamma):
    ...
    for ith_episode in range(1, episodes + 1):
        if ith_episode % 1000 == 0:
            ...
            prob_policy = target_policy(currtne_state)   #get epsilon greedy policy
            ...
            while True:
                        ...
                        next_probbability = create_epsilon_greedy_action_policy(environment,Q,epsilon)(next_state)
...
environment = gym.make('Blackjack-v0')
environment.reset()
Q_SARSA,SARSA = SARSA(environment, 1000000, 0.05, 0.01,0.1)

environment.reset()
aver_payoff(environment,1000,1000,SARSA)
